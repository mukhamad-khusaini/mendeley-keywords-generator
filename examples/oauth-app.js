...
module.exports = function (app, config) {
  ...
  app.get(oauthPath, function (req, res) {
    var authorizationUri = oauth2.authCode.authorizeURL({
      redirect_uri: "http://localhost:8111/examples",
  ...
  app.get("/login", function (req, res) {
    const redirectUrl = new URL("http://localhost:8111" + tokenExchangePath); // Ganti dengan URL tujuan
    redirectUrl.searchParams.append("code", req.query.code);

    // Redirect ke URL tujuan dengan query parameter
