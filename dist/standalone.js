  ...
  root.MendeleySDK.Auth = define(function () {
    ...
    function authCodeFlow(options) {
      ...
      if (!settings.apiAuthenticateUrl) {
        console.error(
          "You must provide an apiAuthenticateUrl for auth code flow"
        );
        return false;
      }
    ...
    function setAccessTokenCookie(accessToken, expireHours) {
      ...
      document.cookie =
        settings.accessTokenCookieName + "=" + accessToken + "; " + expires;
    }
  ...
  root.MendeleySDK.Request = define(function () {
      ...
      if (this.settings.fileUpload) {
        ...
        [
          ...
          xhr.upload.addEventListener(
            uploadEvent,
            uploadProgressFun(dfd, request, xhr)
          );
        });
      }

      $.ajax(request).fail(onFail.bind(this, dfd)).done(onDone.bind(this, dfd));
    ...
    function extractHeaders(xhr) {
      ...
      this.settings.extractHeaders.forEach(function (headerName) {
        headerValue =
          headerName === "Link"
            ? extractLinkHeaders.call(this, xhr)
            : getResponseHeader(xhr, headerName);
    ...
    function extractLinkHeaders(xhr) {
      ...
      var headerName = "Link";
      var links = getAllResponseHeaders(xhr)
        .split("\n")
        .filter(function (row) {
  ...
  root.MendeleySDK.API = define(["./request"], function (Request) {
    ...
    function folders() {
      ...
      return {
         ...
         * @returns {promise}
         */
        retrieveAll: requestFun("GET", "/folders"),
         ...
         * @returns {promise}
         */
        removeDocument: requestFun(
          "DELETE",
          "/folders/{id}/documents/{docId}",
          ["id", "docId"],
          dataHeaders.folder
        ),
         ...
         * @returns {promise}
         */
        addDocument: requestWithDataFun(
          "POST",
          "/folders/{id}/documents",
          ["id"],
          dataHeaders.document,
          false
        ),
         ...
         * @memberof api.folders
         */
        retrievingDocuments: requestFun(
          "GET",
          "/folders/{id}/documents?limit=300",
          ["id"]
        ),
      };
    }
    ...
    function setPaginationLinks(headers) {
      ...
      for (var p in this.paginationLinks) {
        this.paginationLinks[p] = headers.Link.hasOwnProperty(p)
          ? headers.Link[p]
          : false;
      }
    }
    ...
    function uploadHeaders(file, documentId) {
      ...
      if (documentId) {
        headers.Link =
          "<" + apiBaseUrl + "/documents/" + documentId + '>; rel="document"';
      }
